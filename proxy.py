...
proxy = {"http":rand_ip}
print proxy
localurl = urllib.urlopen('https://www.amazon.com/Hyper-Changing-execute-business-intelligence-ebook/dp/B011MXBW96?ie=UTF8&qid=1465090301&ref_=tmm_kin_swatch_0&sr=8-13',proxies=proxy)
...
for rank in ranks:
    ...
    k_rank_array = re.findall('#(\d+)',rank.get_text().replace(',',''))

if not len(k_rank_array):
    print "ranks not retrieved"
else:
    db = MySQLdb.connect("localhost","azim","Az09347108396*","amazon" )
    cursor = db.cursor()
    today = datetime.date.today()
    tstamp = format(datetime.datetime.now())
    sql = """INSERT INTO kindleranks(amz_best_seller_rank, kin_inf_management, books_business, books_inf_management, date, tstamp) VALUES (%s, %s, %s, %s, %s, %s)"""
    cursor.execute(sql,(k_rank_array[0],k_rank_array[1],k_rank_array[2],k_rank_array[3],today,tstamp))
    db.commit()
